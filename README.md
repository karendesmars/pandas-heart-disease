...
- **The final tuned model reaches 88.5% test accuracy** (precision 0.89, recall 0.88, macro average), below the 95% target set in the Evaluation section of the notebook. This target was not reached with the current approach; next steps to close the gap would be collecting more data or trying other model families such as XGBoost or CatBoost.
